(function (chaiModule) {
  ...
  function assertSomething() {
    ...
    expect(object).not.a('string');
    expect(object).is.not.empty;
  ...
  methodNames.forEach(function (methodName) {
    ...
    Assertion.overwriteMethod(methodName, function (_super) {
      return function somethingMethod() {
        ...
        expect(arrayObject).is.not.a('string');

        if (!arrayObject.hasOwnProperty("length")) {
          expect(arrayObject).is.an('object');
          arrayObject = Object.keys(arrayObject).map(function(key) { return arrayObject[key]; });
        }

        expect(arrayObject).is.an('array');
